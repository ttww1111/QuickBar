...
### v1.1.5 (2025-12-27)
- ✅ **原生听写热键重映射 (Agile 逻辑移植)**：
    - 现在 `Win + Alt + H` 已被重映射为触发 Windows 系统原生听写工具。
    - 采用严格的按键序列模拟逻辑，确保在讯飞模式激活时，用户仍能通过组合键无缝切换至系统语音方案。
- ✅ **核心拦截逻辑优化**：
    - 引入按键注入标志位检测，提升了复杂快捷键环境下的拦截稳定性。

### v1.1.4 (2025-12-26)
- ✅ **全新“零闪烁”UI 架构**：
    - 引入局部刷新技术，切换 AI 模式或设置选项时，界面不再闪烁。
    - 优化容器管理逻辑，解决多次切换导致的响应失效问题。
- ✅ **底栏 (Footer) 像素级视觉调优**：
    - **重心校准**：通过对复选框、文字标签和功能图标实施差异化偏移补偿，完美对齐水平中轴线。
    - **间距重塑**：精调模块间水平间距，提升视觉层次感与呼吸感。
- ✅ **自适应高度深度精调**：
    - 修复了单/双列切换时窗口高度不更新的 Bug。
- ✅ **功能修复与稳定性**：
    - 优化了 Win+H 热键在讯飞模式下的拦截稳定性。
