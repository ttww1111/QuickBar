...
### v1.1.3 (2025-12-24)
- ✅ **IDE 环境隔离**：
    - 修正了 Antigravity 模式的窗口匹配逻辑。现在 Antigravity 被视为完全独立的 IDE 容器，其匹配逻辑与 VS Code 严格区分，避免模式选错时指令误发送。
- ✅ **配置一致性优化**：
    - 统一了源码路径与编译后路径下 `target_settings.json` 的自动生成逻辑。
    - 修正了旧版本配置文件中 `win_title` 匹配正则不一致导致的问题。

### v1.1.2 (2025-12-23)
- ✅ **修复校准功能**：解决了编译版本在部分环境下无法保存或读取校准图片的问题。
- ✅ **智能窗口匹配**：
    - 引入基于“窗口类名”的精准识别逻辑，杜绝 VS Code 模式误匹配终端窗口。
    - 适配**管理员模式**运行的 Windows 窗口（支持 `[管理员]` 标题后缀）。
- ✅ **高清图标重制**：生成包含 16~256 全尺寸的高品质多图层 ICO，确保在大图标视图下依然清晰。
- ✅ **交互体验优化**：
    - 重新设计了窗口未找到、识别失败等场景的 UI 提示，增加具体的排查方向指引。
    - 自动化任务过程中增加详细调试信息输出（控制台可见）。
- ✅ **代码健壮性**：增强了 PyAutoGUI 图片识别异常的处理，防止程序因未捕获异常而静默退出。
